...
const modes = ["light", "dark"];
  ...
  return (
      ...
      <div
      className={[
        "font-mono  animate-fade bg-primaryBg h-screen flex flex-col justify-center",
        color && `theme-${color} `,
        mode && `theme-${mode} `,
      ]
        .filter(Boolean)
        .join(" ")}
    >
      <div className="mx-auto bg-neutralBg animate-fade text-onNeutralBg border-8 border-onNeutralBg p-5 max-w-lg">
        ...
        <RadioGroup value={color} onChange={setColor}>
          ...
          <div className="flex justify-between space-x-8 mt-2">
            {colors.map((c) => {
              return (
                <RadioGroup.Option
                  className="ui-checked:text-onPrimaryBg ui-checked:bg-primaryBg ring-4 ui-checked:ring-primary2 ui-not-checked:ring-onNeutralBg h-20 w-full flex justify-center items-center font-bold uppercase cursor-pointer"
                  value={c}
                  key={c}
                >
                  {c}
